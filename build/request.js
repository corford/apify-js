...
class Request {
    ...
    constructor(options = {}) {
        ...
        const { id, url, loadedUrl = null, uniqueKey, method = 'GET', payload = null, noRetry = false, retryCount = 0, errorMessages = null, headers = {}, userData = {}, handledAt = null, keepUrlFragment = false, useExtendedUniqueKey = false, } = options;
        utils_1.checkParamOrThrow(id, 'id', 'Maybe String');
        utils_1.checkParamOrThrow(url, 'url', 'String');
        utils_1.checkParamOrThrow(loadedUrl, 'url', 'Maybe String');
        utils_1.checkParamOrThrow(uniqueKey, 'uniqueKey', 'Maybe String');
        utils_1.checkParamOrThrow(method, 'method', 'String');
        utils_1.checkParamOrThrow(payload, 'payload', 'Maybe Buffer | String');
        utils_1.checkParamOrThrow(noRetry, 'noRetry', 'Boolean');
        utils_1.checkParamOrThrow(retryCount, 'retryCount', 'Number');
        utils_1.checkParamOrThrow(errorMessages, 'errorMessages', 'Maybe Array');
        utils_1.checkParamOrThrow(headers, 'headers', 'Object');
        utils_1.checkParamOrThrow(userData, 'userData', 'Object');
        utils_1.checkParamOrThrow(handledAt, 'handledAt', 'Maybe String | Date');
        utils_1.checkParamOrThrow(keepUrlFragment, 'keepUrlFragment', 'Boolean');
        utils_1.checkParamOrThrow(useExtendedUniqueKey, 'useExtendedUniqueKey', 'Boolean');
        if (method === 'GET' && payload)
            throw new Error('Request with GET method cannot have a payload.');
        if (!url)
            throw new Error('The "url" option cannot be empty string.');
        this.id = id;
        this.url = url;
