...
exports.BROWSER_SESSION_KEY_NAME = void 0;
...
exports.BROWSER_SESSION_KEY_NAME = 'APIFY_SESSION';
...
class PuppeteerPool {
    ...
    constructor(options = {}) {
        ...
        maxOpenPagesPerInstance = 50, retireInstanceAfterRequestCount = 100, launchPuppeteerFunction = launchPuppeteerOptions => puppeteer_1.launchPuppeteer(launchPuppeteerOptions), puppeteerOperationTimeoutSecs = 15, instanceKillerIntervalMillis, instanceKillerIntervalSecs = 60, killInstanceAfterMillis, killInstanceAfterSecs = 300, launchPuppeteerOptions, recycleDiskCache = false, useIncognitoPages = false, proxyUrls, proxyConfiguration, useLiveView = false, sessionPool, log = utils_log_1.default, } = options;
        // Disabling due to memory leak.
        const reusePages = false;
        utils_1.checkParamOrThrow(reusePages, 'options.reusePages', 'Boolean');
        utils_1.checkParamOrThrow(maxOpenPagesPerInstance, 'options.maxOpenPagesPerInstance', 'Number');
        utils_1.checkParamOrThrow(retireInstanceAfterRequestCount, 'options.retireInstanceAfterRequestCount', 'Number');
        utils_1.checkParamOrThrow(launchPuppeteerFunction, 'options.launchPuppeteerFunction', 'Function');
        utils_1.checkParamOrThrow(puppeteerOperationTimeoutSecs, 'options.puppeteerOperationTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(instanceKillerIntervalMillis, 'options.instanceKillerIntervalMillis', 'Maybe Number');
        ...
        utils_1.checkParamOrThrow(instanceKillerIntervalSecs, 'options.instanceKillerIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(killInstanceAfterMillis, 'options.killInstanceAfterMillis', 'Maybe Number');
        ...
        utils_1.checkParamOrThrow(killInstanceAfterSecs, 'options.killInstanceAfterSecs', 'Number');
        utils_1.checkParamOrThrow(launchPuppeteerOptions, 'options.launchPuppeteerOptions', 'Maybe Object');
        utils_1.checkParamOrThrow(recycleDiskCache, 'options.recycleDiskCache', 'Maybe Boolean');
        utils_1.checkParamOrThrow(useIncognitoPages, 'options.useIncognitoPages', 'Maybe Boolean');
        utils_1.checkParamOrThrow(proxyUrls, 'options.proxyUrls', 'Maybe Array');
        ...
        utils_1.checkParamOrThrow(useLiveView, 'options.useLiveView', 'Maybe Boolean');
        utils_1.checkParamOrThrow(sessionPool, 'options.sessionPool', 'Maybe Object');
        // Config.
        ...
        this.launchPuppeteerFunction = async () => {
            ...
            if (session)
                browser[exports.BROWSER_SESSION_KEY_NAME] = session;
            return browser;
        };
        // State.
    ...
    async _initBrowser(browserPromise, instance) {
        ...
        try {
            ...
            if (this.sessionPool) {
                instance.session = browser[exports.BROWSER_SESSION_KEY_NAME];
            }
        }
        catch (err) {
    ...
    async _openNewTab() {
        ...
        try {
            ...
            const page = await utils_2.addTimeoutToPromise(context.newPage(), this.puppeteerOperationTimeoutMillis, 'browser.newPage() timed out.');
    ...
    async recyclePage(page) {
        ...
        if (this.reusePages) {
            ...
            try {
                await utils_2.addTimeoutToPromise(page.close(), this.puppeteerOperationTimeoutMillis, 'page.close() timed out.');
            }
            catch (err) {
