...
class RequestList {
    ...
    constructor(options = {}) {
        ...
        const { sources, sourcesFunction, persistStateKey, persistRequestsKey, persistSourcesKey, state, keepDuplicateUrls = false, } = options;
        ...
        utils_1.checkParamOrThrow(sources, 'options.sources', 'Maybe Array');
        utils_1.checkParamOrThrow(sourcesFunction, 'options.sourcesFunction', 'Maybe Function');
        utils_1.checkParamOrThrow(state, 'options.state', 'Maybe Object');
        utils_1.checkParamOrThrow(persistStateKey, 'options.persistStateKey', 'Maybe String');
        utils_1.checkParamOrThrow(persistRequestsKey, 'options.persistRequestsKey', 'Maybe String');
        utils_1.checkParamOrThrow(persistSourcesKey, 'options.persistSourcesKey', 'Maybe String');
        utils_1.checkParamOrThrow(keepDuplicateUrls, 'options.keepDuplicateUrls', 'Maybe Boolean');
    ...
    async isEmpty() {
        this._ensureIsInitialized();
        return !utils_2.getFirstKey(this.reclaimed) && this.nextIndex >= this.requests.length;
    }
    /**
    ...
    async isFinished() {
        this._ensureIsInitialized();
        return !utils_2.getFirstKey(this.inProgress) && this.nextIndex >= this.requests.length;
    }
    /**
    ...
    async fetchNextRequest() {
        ...
        const uniqueKey = utils_2.getFirstKey(this.reclaimed);
        if (uniqueKey) {
    ...
    async _fetchRequestsFromUrl(source) {
        ...
        const { requestsFromUrl, regex } = source;
        const { downloadListOfUrls } = utils_2.publicUtils;
...
exports.openRequestList = async (listName, sources, options = {}) => {
    utils_1.checkParamOrThrow(listName, 'listName', 'String | Null');
    utils_1.checkParamOrThrow(sources, 'sources', 'Array');
    utils_1.checkParamOrThrow(options, 'options', 'Object');
    const rl = new RequestList({
        ...options,
        persistStateKey: listName ? `${listName}-${exports.STATE_PERSISTENCE_KEY}` : null,
        persistRequestsKey: listName ? `${listName}-${exports.REQUESTS_PERSISTENCE_KEY}` : null,
        sources,
    });
    await rl.initialize();
    return rl;
