...
const global_cache_1 = require("./global_cache");
const utils_log_1 = require("./utils_log");
exports.LOCAL_STORAGE_SUBDIR = consts_1.LOCAL_STORAGE_SUBDIRS.keyValueStores;
const MAX_OPENED_STORES = 1000;
const DEFAULT_LOCAL_FILE_EXTENSION = 'bin';
const COMMON_LOCAL_FILE_EXTENSIONS = ['bin', 'txt', 'json', 'html', 'xml', 'jpeg', 'png', 'pdf', 'mp3', 'js', 'css', 'csv'];
const readFilePromised = util_1.promisify(fs.readFile);
const readdirPromised = util_1.promisify(fs.readdir);
const writeFilePromised = util_1.promisify(fs.writeFile);
const unlinkPromised = util_1.promisify(fs.unlink);
const statPromised = util_1.promisify(fs.stat);
const emptyDirPromised = util_1.promisify(fs.emptyDir);
const { keyValueStores } = utils_2.apifyClient;
const storesCache = global_cache_1.default.create('key-value-store-cache', MAX_OPENED_STORES); // Open key-value stores are stored here.
/**
...
const validateGetValueParams = (key) => {
    utils_1.checkParamOrThrow(key, 'key', 'String');
    if (!key)
...
const validateSetValueParams = (key, value, options) => {
    utils_1.checkParamOrThrow(key, 'key', 'String');
    utils_1.checkParamOrThrow(options, 'options', 'Object');
    utils_1.checkParamOrThrow(options.contentType, 'options.contentType', 'String | Null | Undefined');
    ...
    if (options.contentType) {
        utils_1.checkParamOrThrow(value, 'value', 'Buffer | String', 'The "value" parameter must be a String or Buffer when "options.contentType" is specified.'); // eslint-disable-line max-len
    }
    if (options.contentType === '')
        throw new Error('Parameter options.contentType cannot be empty string.');
    if (!key)
        throw new Error('The "key" parameter cannot be empty');
    if (!regexs_1.KEY_VALUE_STORE_KEY_REGEX.test(key)) {
...
class KeyValueStore {
    ...
    constructor(storeId, storeName) {
        utils_1.checkParamOrThrow(storeId, 'storeId', 'String');
        utils_1.checkParamOrThrow(storeName, 'storeName', 'Maybe String');
    ...
    setValue(key, value, options = {}) {
        ...
        if (value === null)
            return keyValueStores.deleteRecord({ storeId: this.storeId, key });
        ...
        return keyValueStores.putRecord({
            ...
            key,
            body: value,
            contentType: utils_2.addCharsetToContentType(optionsCopy.contentType),
        });
    }
    /**
     ...
     */
    getPublicUrl(key) {
        return `${constants_1.APIFY_API_BASE_URL}/key-value-stores/${this.storeId}/records/${key}`;
    }
    /**
    ...
    async forEachKey(iteratee, options = {}, index = 0) {
        ...
        utils_1.checkParamOrThrow(iteratee, 'iteratee', 'Function');
        utils_1.checkParamOrThrow(exclusiveStartKey, 'options.exclusiveStartKey', 'Maybe String');
        utils_1.checkParamOrThrow(index, 'index', 'Number');
...
class KeyValueStoreLocal {
    constructor(storeId, localStorageDir) {
        utils_1.checkParamOrThrow(storeId, 'storeId', 'String');
        utils_1.checkParamOrThrow(localStorageDir, 'localStorageDir', 'String');
        this.log = utils_log_1.default.child({ prefix: 'KeyValueStore' });
        this.localStoragePath = path.resolve(path.join(localStorageDir, exports.LOCAL_STORAGE_SUBDIR, storeId));
        this.storeId = storeId;
        this.initializationPromise = utils_2.ensureDirExists(this.localStoragePath);
    }
    async getValue(key) {
        validateGetValueParams(key);
        ...
        try {
            ...
            return result
                ? utils_1.parseBody(result.returnValue, mime.contentType(result.fileName))
                : null;
        }
        catch (err) {
            throw new Error(`Error reading file '${key}' in directory '${this.localStoragePath}' referred by ${consts_1.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line
        }
    }
    async setValue(key, value, options = {}) {
        ...
        try {
            ...
            throw new Error(`Error removing file '${key}' in directory '${this.localStoragePath}' referred by ${consts_1.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line max-len
        }
        ...
        try {
            ...
            throw new Error(`Error writing file '${key}' in directory '${this.localStoragePath}' referred by ${consts_1.ENV_VARS.LOCAL_STORAGE_DIR} environment variable: ${err.message}`); // eslint-disable-line max-len
        }
    }
    async delete() {
        this.log.deprecated('keyValueStore.delete() is deprecated. Please use keyValueStore.drop() instead. '
    ...
    _fullDirectoryLookup(key, handler) {
        return readdirPromised(this.localStoragePath)
            .then((files) => {
            const regex = exports.getFileNameRegexp(key);
 ...
 * @function
 */
exports.openKeyValueStore = (storeIdOrName, options = {}) => {
    utils_1.checkParamOrThrow(storeIdOrName, 'storeIdOrName', 'Maybe String');
    utils_1.checkParamOrThrow(options, 'options', 'Object');
    utils_2.ensureTokenOrLocalStorageEnvExists('key-value store');
    const { forceCloud = false } = options;
    utils_1.checkParamOrThrow(forceCloud, 'options.forceCloud', 'Boolean');
    return process.env[consts_1.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud
        ? utils_2.openLocalStorage(storeIdOrName, consts_1.ENV_VARS.DEFAULT_KEY_VALUE_STORE_ID, KeyValueStoreLocal, storesCache)
        : utils_2.openRemoteStorage(storeIdOrName, consts_1.ENV_VARS.DEFAULT_KEY_VALUE_STORE_ID, KeyValueStore, storesCache, getOrCreateKeyValueStore);
};
/**
