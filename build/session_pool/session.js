...
class Session {
     ...
     */
    constructor(options = {}) {
        const { id = `session_${utilities_1.cryptoRandomObjectId(10)}`, cookieJar = new tough_cookie_1.CookieJar(), maxAgeSecs = DEFAULT_SESSION_MAX_AGE_SECS, userData = {}, maxErrorScore = 3, errorScoreDecrement = 0.5, createdAt = new Date(), usageCount = 0, errorScore = 0, maxUsageCount = 50, sessionPool, log = utils_log_1.default, } = options;
        const { expiresAt = this._getDefaultCookieExpirationDate(maxAgeSecs) } = options;
        // Validation
        utils_1.checkParamOrThrow(id, 'options.id', 'String');
        utils_1.checkParamOrThrow(maxAgeSecs, 'options.maxAgeSecs', 'Number');
        utils_1.checkParamOrThrow(userData, 'options.userData', 'Object');
        utils_1.checkParamOrThrow(maxErrorScore, 'options.maxErrorScore', 'Number');
        utils_1.checkParamOrThrow(expiresAt, 'options.expiresAt', 'Maybe Date');
        utils_1.checkParamOrThrow(createdAt, 'options.createdAt', 'Date');
        utils_1.checkParamOrThrow(usageCount, 'options.usageCount', 'Number');
        utils_1.checkParamOrThrow(errorScore, 'options.errorScore', 'Number');
        utils_1.checkParamOrThrow(maxUsageCount, 'options.maxUsageCount', 'Number');
        utils_1.checkParamOrThrow(sessionPool, 'options.sessionPool', 'Object');
