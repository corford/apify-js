...
const request_queue_1 = require("../request_queue"); // eslint-disable-line import/no-duplicates
...
class BasicCrawler {
    ...
    constructor(options) {
        ...
        }, maxRequestRetries = 3, maxRequestsPerCrawl, proxyConfiguration, autoscaledPoolOptions = {}, sessionPoolOptions = {}, useSessionPool = false, 
        // AutoscaledPool shorthands
        minConcurrency, maxConcurrency, 
        // internal
        log = utils_log_1.default.child({ prefix: 'BasicCrawler' }), } = options;
        utilities_1.checkParamPrototypeOrThrow(requestList, 'options.requestList', request_list_1.RequestList, 'Apify.RequestList', true);
        utilities_1.checkParamPrototypeOrThrow(proxyConfiguration, 'options.proxyConfiguration', proxy_configuration_1.ProxyConfiguration, 'Apify.proxyConfiguration', true);
        utilities_1.checkParamPrototypeOrThrow(requestQueue, 'options.requestQueue', [request_queue_1.RequestQueue, request_queue_1.RequestQueueLocal], 'Apify.RequestQueue', true);
        utils_1.checkParamOrThrow(handleRequestFunction, 'options.handleRequestFunction', 'Function');
        utils_1.checkParamOrThrow(handleRequestTimeoutSecs, 'options.handleRequestTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(handleFailedRequestFunction, 'options.handleFailedRequestFunction', 'Function');
        utils_1.checkParamOrThrow(maxRequestRetries, 'options.maxRequestRetries', 'Number');
        utils_1.checkParamOrThrow(maxRequestsPerCrawl, 'options.maxRequestsPerCrawl', 'Maybe Number');
        utils_1.checkParamOrThrow(autoscaledPoolOptions, 'options.autoscaledPoolOptions', 'Object');
    ...
    async _runTaskFunction() {
        ...
        try {
            await utils_2.addTimeoutToPromise(this.handleRequestFunction({ request, autoscaledPool: this.autoscaledPool, session, proxyInfo }), this.handleRequestTimeoutMillis, `handleRequestFunction timed out after ${this.handleRequestTimeoutMillis / 1000} seconds.`);
