...
class CheerioCrawler {
     ...
     */
    constructor(options = {}) {
        const { requestOptions, handlePageFunction, parseContentTypeFunction = this._defaultParseContentTypeFunction, requestTimeoutSecs = 30, handlePageTimeoutSecs = 60, ignoreSslErrors = true, proxyUrls, additionalMimeTypes = [], suggestResponseEncoding, forceResponseEncoding, proxyConfiguration, 
        // Autoscaled pool shorthands
        minConcurrency, maxConcurrency, 
        // Basic crawler options
        requestList, requestQueue, maxRequestRetries, maxRequestsPerCrawl, handleFailedRequestFunction = this._defaultHandleFailedRequestFunction.bind(this), autoscaledPoolOptions = DEFAULT_AUTOSCALED_POOL_OPTIONS, prepareRequestFunction, useSessionPool = false, sessionPoolOptions = {}, persistCookiesPerSession = false, } = options;
        utils_1.checkParamOrThrow(handlePageFunction, 'options.handlePageFunction', 'Function');
        utils_1.checkParamOrThrow(requestOptions, 'options.requestOptions', 'Maybe Object');
        utils_1.checkParamOrThrow(requestTimeoutSecs, 'options.requestTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(handlePageTimeoutSecs, 'options.handlePageTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(ignoreSslErrors, 'options.ignoreSslErrors', 'Maybe Boolean');
        utils_1.checkParamOrThrow(proxyUrls, 'options.proxyUrls', 'Maybe [String]');
        utils_1.checkParamOrThrow(prepareRequestFunction, 'options.prepareRequestFunction', 'Maybe Function');
        utils_1.checkParamOrThrow(additionalMimeTypes, 'options.additionalMimeTypes', '[String]');
        utils_1.checkParamOrThrow(suggestResponseEncoding, 'options.suggestResponseEncoding', 'Maybe String');
        utils_1.checkParamOrThrow(forceResponseEncoding, 'options.forceResponseEncoding', 'Maybe String');
        utils_1.checkParamOrThrow(useSessionPool, 'options.useSessionPool', 'Boolean');
        utils_1.checkParamOrThrow(sessionPoolOptions, 'options.sessionPoolOptions', 'Object');
        utils_1.checkParamOrThrow(persistCookiesPerSession, 'options.persistCookiesPerSession', 'Boolean');
        ...
        if (proxyConfiguration && proxyUrls)
            throw new Error('Cannot combine "options.proxyConfiguration" with "options.proxyUrls"!');
     ...
     */
    use(extension) {
        const inheritsFromCrawlerExtension = extension instanceof crawler_extension_1.default;
    ...
    async _handleRequestFunction({ request, autoscaledPool, session, proxyInfo }) {
        if (this.prepareRequestFunction)
            await this.prepareRequestFunction({ request, session });
        const { dom, isXml, body, contentType, response } = await utils_2.addTimeoutToPromise(this._requestFunction({ request, session }), this.requestTimeoutMillis, `request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);
        ...
        return utils_2.addTimeoutToPromise(this.handlePageFunction(context), this.handlePageTimeoutMillis, `handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
    }
    /**
    ...
    _getRequestOptions(request, session) {
        const mandatoryRequestOptions = {
            ...
            abortFunction: (res) => {
                ...
                const { type } = utils_2.parseContentTypeFromResponse(res);
                if (statusCode === 406) {
    ...
    _defaultParseContentTypeFunction(request, responseStream) {
        return utils_2.parseContentTypeFromResponse(responseStream);
    }
}
