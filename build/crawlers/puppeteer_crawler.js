...
class PuppeteerCrawler {
    ...
    constructor(options) {
        ...
        utils_1.checkParamOrThrow(gotoFunction, 'options.gotoFunction', 'Function');
        utils_1.checkParamOrThrow(gotoTimeoutSecs, 'options.gotoTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(puppeteerPoolOptions, 'options.puppeteerPoolOptions', 'Maybe Object');
    ...
    async _handleRequestFunction({ request, autoscaledPool }) {
        ...
        if (this.sessionPool) {
            ...
            session = browser[puppeteer_pool_1.BROWSER_SESSION_KEY_NAME];
        ...
        try {
            ...
            await utils_2.addTimeoutToPromise(this.handlePageFunction({ page, request, autoscaledPool, puppeteerPool: this.puppeteerPool, response, session, proxyInfo }), this.handlePageTimeoutMillis, `handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
            if (session)
                session.markGood();
        }
        finally {
