...
class ProxyConfiguration {
     ...
     */
    constructor(options = {}) {
        const { groups = [], apifyProxyGroups = [], countryCode, apifyProxyCountry, password = process.env[consts_1.ENV_VARS.PROXY_PASSWORD], } = options;
        ...
        // Validation
        utils_1.checkParamOrThrow(groupsToUse, 'opts.groups', '[String]');
        utils_1.checkParamOrThrow(countryCodeToUse, 'opts.countryCode', 'Maybe String');
        utils_1.checkParamOrThrow(password, 'opts.password', 'Maybe String');
    ...
    getInfo(sessionId) {
        if (sessionId)
            this._validateSessionArgumentStructure(sessionId);
    ...
    async _setPasswordIfToken() {
        ...
        if (token) {
            const { proxy: { password } } = await utils_2.apifyClient.users.getUser({ token, userId: 'me' });
            if (this.password) {
                if (this.password !== password) {
                    utils_log_1.default.warning('The Apify Proxy password you provided belongs to'
    ...
    async _checkAccess() {
        ...
        const { body: { connected, connectionError } } = await utils_request_1.requestAsBrowser({ url, proxyUrl, countryCode, json: true });
        if (!connected)
    ...
    _validateSessionArgumentStructure(sessionId) {
        if (!regexs_1.APIFY_PROXY_VALUE_REGEX.test(sessionId))
            this._throwInvalidProxyValueError(sessionId);
    }
    /**
    ...
    _validateArgumentStructure(groups, countryCode) {
        for (const group of groups) {
            if (!regexs_1.APIFY_PROXY_VALUE_REGEX.test(group))
                this._throwInvalidProxyValueError(group);
        }
        if (countryCode) {
            if (!constants_1.COUNTRY_CODE_REGEX.test(countryCode))
                this._throwInvalidCountryCode(countryCode);
        }
    }
    /**
