...
const addInputOptionsOrThrow = (input, contentType, options) => {
    ...
    options.body = key_value_store_1.maybeStringify(input, options);
    utils_1.checkParamOrThrow(options.body, 'input', 'Buffer|String');
    utils_1.checkParamOrThrow(options.contentType, 'contentType', 'String');
    options.contentType = utils_2.addCharsetToContentType(options.contentType);
};
/**
...
exports.getEnv = () => {
    ...
    _.mapObject(consts_1.ENV_VARS, (fullName, shortName) => {
        const camelCaseName = utils_2.snakeCaseToCamelCase(shortName);
...
exports.main = (userFunc) => {
    ...
    utils_2.logSystemInfo();
    // Log warning if SDK is outdated.
    utils_2.printOutdatedSdkWarning();
...
exports.call = async (actId, input, options = {}) => {
    const { acts, keyValueStores } = utils_2.apifyClient;
    utils_1.checkParamOrThrow(actId, 'actId', 'String');
    utils_1.checkParamOrThrow(options, 'opts', 'Object');
    // Common options.
    const { token } = options;
    utils_1.checkParamOrThrow(token, 'token', 'Maybe String');
    // RunAct() options.
    ...
    if (memory && !memoryMbytes) {
        ...
        if (!callMemoryWarningIssued) {
            ...
            utils_log_1.default.warning('The "memory" option of the Apify.call() function has been deprecated and will be removed in the future. Use "memoryMbytes" instead!');
        }
    }
    utils_1.checkParamOrThrow(build, 'build', 'Maybe String');
    utils_1.checkParamOrThrow(memoryMbytes, 'memoryMbytes', 'Maybe Number');
    utils_1.checkParamOrThrow(timeoutSecs, 'timeoutSecs', 'Maybe Number');
    utils_1.checkParamOrThrow(webhooks, 'webhooks', 'Maybe Array');
    if (token)
        runActOpts.token = token;
    if (build)
        runActOpts.build = build;
    if (memoryMbytes)
        runActOpts.memory = memoryMbytes;
    if (timeoutSecs >= 0)
        runActOpts.timeout = timeoutSecs; // Zero is valid value!
    if (webhooks)
        runActOpts.webhooks = webhooks;
    if (input)
        addInputOptionsOrThrow(input, options.contentType, runActOpts);
    // Run actor.
    const { waitSecs } = options;
    utils_1.checkParamOrThrow(waitSecs, 'waitSecs', 'Maybe Number');
    const run = await acts.runAct(runActOpts);
    if (waitSecs <= 0)
        return run; // In this case there is nothing more to do.
    // Wait for run to finish.
    let updatedRun;
    try {
        updatedRun = await utils_2.waitForRunToFinish({
            actorId: actId,
            runId: run.id,
            token,
            waitSecs,
    ...
    if (isRunUnsuccessful(updatedRun.status)) {
        ...
        throw new errors_1.ApifyCallError(updatedRun, message);
    }
    // Finish if output is not requested or run haven't finished.
    const { fetchOutput = true } = options;
    if (!fetchOutput || updatedRun.status !== consts_1.ACT_JOB_STATUSES.SUCCEEDED)
        return updatedRun;
    // Fetch output.
    const { disableBodyParser = false } = options;
    utils_1.checkParamOrThrow(disableBodyParser, 'disableBodyParser', 'Boolean');
...
exports.callTask = async (taskId, input, options = {}) => {
    const { tasks, keyValueStores } = utils_2.apifyClient;
    utils_1.checkParamOrThrow(taskId, 'taskId', 'String');
    utils_1.checkParamOrThrow(options, 'opts', 'Object');
    // Common options.
    const { token } = options;
    utils_1.checkParamOrThrow(token, 'token', 'Maybe String');
    ...
    utils_1.checkParamOrThrow(webhooks, 'webhooks', 'Maybe Array');
    if (token)
        runTaskOpts.token = token;
    if (build)
        runTaskOpts.build = build;
    if (memoryMbytes)
        runTaskOpts.memory = memoryMbytes;
    if (timeoutSecs >= 0)
        runTaskOpts.timeout = timeoutSecs; // Zero is valid value!
    if (input)
        runTaskOpts.input = input;
    if (webhooks)
        runTaskOpts.webhooks = webhooks;
    // Start task.
    const { waitSecs } = options;
    utils_1.checkParamOrThrow(waitSecs, 'waitSecs', 'Maybe Number');
...
exports.metamorph = async (targetActorId, input, options = {}) => {
    ...
    const { acts } = utils_2.apifyClient;
    utils_1.checkParamOrThrow(targetActorId, 'targetActorId', 'String');
    utils_1.checkParamOrThrow(optionsCopy, 'opts', 'Object');
    utils_1.checkParamOrThrow(optionsCopy.build, 'options.build', 'Maybe String');
    utils_1.checkParamOrThrow(optionsCopy.contentType, 'options.contentType', 'Maybe String');
    ...
    if (input) {
        input = key_value_store_1.maybeStringify(input, optionsCopy);
        utils_1.checkParamOrThrow(input, 'input', 'Buffer|String');
        if (optionsCopy.contentType)
            optionsCopy.contentType = utils_2.addCharsetToContentType(optionsCopy.contentType);
    }
    ...
    await utils_2.sleep(optionsCopy.customAfterSleepMillis || METAMORPH_AFTER_SLEEP_MILLIS);
};
/**
 *
 ...
 * @name addWebhook
 */
exports.addWebhook = async ({ eventTypes, requestUrl, payloadTemplate, idempotencyKey }) => {
    utils_1.checkParamOrThrow(eventTypes, 'eventTypes', '[String]');
    utils_1.checkParamOrThrow(requestUrl, 'requestUrl', 'String');
    utils_1.checkParamOrThrow(payloadTemplate, 'payloadTemplate', 'Maybe String');
    if (!utils_2.isAtHome()) {
    ...
    return utils_2.apifyClient.webhooks.createWebhook({
        webhook: {
            isAdHoc: true,
            eventTypes,
            condition: {
