...
exports.LOCAL_STORAGE_SUBDIR = consts_1.LOCAL_STORAGE_SUBDIRS.requestQueues;
...
const { requestQueues } = utils_2.apifyClient;
const queuesCache = global_cache_1.default.create('request-queue-cache', MAX_OPENED_QUEUES); // Open queues are stored here.
/**
...
const validateAddRequestParams = (request, opts) => {
    ...
    utils_1.checkParamOrThrow(opts, 'opts', 'Object');
    const newRequest = request instanceof request_1.default ? request : new request_1.default(request);
    const { forefront = false } = opts;
    utils_1.checkParamOrThrow(forefront, 'opts.forefront', 'Boolean');
    if (request.id)
        throw new Error('Request already has the "id" field set so it cannot be added to the queue!');
...
const validateGetRequestParams = (requestId) => {
    utils_1.checkParamOrThrow(requestId, 'requestId', 'String');
};
/**
...
const validateMarkRequestHandledParams = (request) => {
    utilities_1.checkParamPrototypeOrThrow(request, 'request', request_1.default, 'Apify.Request');
    utils_1.checkParamOrThrow(request.id, 'request.id', 'String');
};
/**
...
const getRequestId = (uniqueKey) => {
    utils_1.checkParamOrThrow(uniqueKey, 'uniqueKey', 'String');
    const str = crypto
        .createHash('sha256')
        .update(uniqueKey)
        .digest('base64')
...
class RequestQueue {
     ...
     */
    constructor(queueId, queueName, clientKey = utilities_1.cryptoRandomObjectId()) {
        utils_1.checkParamOrThrow(queueId, 'queueId', 'String');
        utils_1.checkParamOrThrow(queueName, 'queueName', 'Maybe String');
        utils_1.checkParamOrThrow(clientKey, 'clientKey', 'String');
        if (!clientKey)
            throw new Error('Parameter "clientKey" must be a non-empty string!');
    ...
    async getRequest(requestId) {
        ...
        return obj ? new request_1.default(obj) : null;
    }
    /**
    ...
    _cacheRequest(cacheKey, queueOperationInfo) {
        utils_1.checkParamOrThrow(cacheKey, 'cacheKey', 'String');
        utils_1.checkParamOrThrow(queueOperationInfo, 'queueOperationInfo', 'Object');
        utils_1.checkParamOrThrow(queueOperationInfo.requestId, 'queueOperationInfo.requestId', 'String');
        utils_1.checkParamOrThrow(queueOperationInfo.wasAlreadyHandled, 'queueOperationInfo.wasAlreadyHandled', 'Boolean');
    ...
    async _ensureHeadIsNonEmpty(ensureConsistency = false, limit = Math.max(this.inProgressCount() * exports.QUERY_HEAD_BUFFER, exports.QUERY_HEAD_MIN_LENGTH), iteration = 0) {
        utils_1.checkParamOrThrow(ensureConsistency, 'ensureConsistency', 'Boolean');
        utils_1.checkParamOrThrow(limit, 'limit', 'Number');
        utils_1.checkParamOrThrow(iteration, 'iteration', 'Number');
        ...
        if (shouldRepeatForConsistency) {
            ...
            await utils_2.sleep(delayMillis);
        }
...
class RequestQueueLocal {
    constructor(queueId, localStorageDir) {
        ...
        this.log = utils_log_1.default.child({ prefix: 'RequestQueue' });
        this.queueId = queueId;
        this.localStoragePath = path.resolve(path.join(localStorageDir, exports.LOCAL_STORAGE_SUBDIR, queueId));
    ...
    async _initialize() {
        ...
        await utils_2.ensureDirExists(this.localHandledEmulationPath);
        await utils_2.ensureDirExists(this.localPendingEmulationPath);
    ...
    async _getRequestByQueueOrderNo(queueOrderNo) {
        utils_1.checkParamOrThrow(queueOrderNo, 'queueOrderNo', 'Number');
        let buffer;
        let filePath;
        try {
        ...
        return new request_1.default(requestOptions);
    }
    ...
    async getInfo() {
        ...
        const name = id === consts_1.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID ? null : id;
        const result = {
            id,
            name,
 ...
 * @function
 */
exports.openRequestQueue = (queueIdOrName, options = {}) => {
    utils_1.checkParamOrThrow(queueIdOrName, 'queueIdOrName', 'Maybe String');
    utils_1.checkParamOrThrow(options, 'options', 'Object');
    utils_2.ensureTokenOrLocalStorageEnvExists('request queue');
    ...
    return process.env[consts_1.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud
        ? utils_2.openLocalStorage(queueIdOrName, consts_1.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID, RequestQueueLocal, queuesCache)
        : utils_2.openRemoteStorage(queueIdOrName, consts_1.ENV_VARS.DEFAULT_REQUEST_QUEUE_ID, RequestQueue, queuesCache, getOrCreateQueue);
};
