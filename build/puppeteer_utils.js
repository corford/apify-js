...
const hideWebDriver = async (page) => {
    utils_log_1.default.deprecated('Apify.utils.puppeteer.hideWebDriver() is deprecated. Use launchPuppeteerOptions.stealth instead.');
    utils_1.checkParamOrThrow(page, 'page', 'Object');
...
const injectFile = async (page, filePath, options = {}) => {
    utils_1.checkParamOrThrow(page, 'page', 'Object');
    utils_1.checkParamOrThrow(filePath, 'filePath', 'String');
...
const enqueueRequestsFromClickableElements = async (page, selector, purls, requestQueue, requestOpts = {}) => {
    utils_log_1.default.warning('Function enqueueRequestsFromClickableElements is deprecated!!! Use `enqueueLinks` instead!');
    utils_1.checkParamOrThrow(page, 'page', 'Object');
    utils_1.checkParamOrThrow(purls, 'purls', 'Array');
    ...
    utils_1.checkParamOrThrow(requestOpts, 'requestOpts', 'Object');
    ...
    const requests = urls.filter(matchesPseudoUrl).map(url => new request_1.default(Object.assign({ url }, requestOpts)));
...
const blockRequests = async (page, options = {}) => {
    ...
    utils_1.checkParamOrThrow(urlPatterns, 'options.urlPatterns', '[String]');
    utils_1.checkParamOrThrow(extraUrlPatterns, 'options.extraUrlPatterns', '[String]');
...
const cacheResponses = async (page, cache, responseUrlRules) => {
    utils_1.checkParamOrThrow(page, 'page', 'Object');
    utils_1.checkParamOrThrow(cache, 'cache', 'Object');
    utils_1.checkParamOrThrow(responseUrlRules, 'responseUrlRules', 'Array');
    ...
    responseUrlRules.forEach((rule, index) => utils_1.checkParamOrThrow(rule, `responseUrlRules[${index}]`, 'String | RegExp'));
    await puppeteer_request_interception_1.addInterceptRequestHandler(page, async (request) => {
...
exports.gotoExtended = async (page, request, gotoOptions = {}) => {
    ...
    utils_1.checkParamOrThrow(gotoOptions, 'gotoOptions', 'Object');
...
exports.infiniteScroll = async (page, options = {}) => {
    ...
    utils_1.checkParamOrThrow(page, 'page', 'Object');
    utils_1.checkParamOrThrow(timeoutSecs, 'timeoutSecs', 'Number');
    utils_1.checkParamOrThrow(waitForSecs, 'waitForSecs', 'Number');
    let finished;
...
const saveSnapshot = async (page, options = {}) => {
    ...
    try {
        ...
        utils_1.checkParamOrThrow(saveScreenshot, 'saveScreenshot', 'Boolean');
        utils_1.checkParamOrThrow(saveHtml, 'saveHtml', 'Boolean');
        utils_1.checkParamOrThrow(key, 'key', 'String');
        utils_1.checkParamOrThrow(keyValueStoreName, 'keyValueStoreName', 'Maybe String');
        utils_1.checkParamOrThrow(screenshotQuality, 'screenshotQuality', 'Number');
        const store = await key_value_store_1.openKeyValueStore(keyValueStoreName);
        if (saveScreenshot) {
            const screenshotName = utils_2.isAtHome() ? `${key}.jpg` : key;
        ...
        if (saveHtml) {
            const htmlName = utils_2.isAtHome() ? `${key}.html` : key;
        ...
        utils_log_1.default.error(`saveSnapshot with key ${key || ''} failed with error:`);
        throw e;
    }
};
