...
exports.LOCAL_STORAGE_SUBDIR = consts_1.LOCAL_STORAGE_SUBDIRS.datasets;
exports.LOCAL_FILENAME_DIGITS = 9;
exports.LOCAL_GET_ITEMS_DEFAULT_LIMIT = 250000;
...
const getLocaleFilename = index => `${utilities_1.leftpad(index, exports.LOCAL_FILENAME_DIGITS, 0)}.json`;
const { datasets } = utils_2.apifyClient;
const datasetsCache = global_cache_1.default.create('dataset-cache', MAX_OPENED_DATASETS); // Open Datasets are stored here.
/**
...
exports.checkAndSerialize = (item, limitBytes, index) => {
    ...
    let payload;
    try {
        utils_1.checkParamOrThrow(item, 'item', 'Object');
...
class Dataset {
    ...
    constructor(datasetId, datasetName) {
        utils_1.checkParamOrThrow(datasetId, 'datasetId', 'String');
        utils_1.checkParamOrThrow(datasetName, 'datasetName', 'Maybe String');
     ...
     */
    async pushData(data) {
        utils_1.checkParamOrThrow(data, 'data', 'Array | Object');
...
class DatasetLocal {
    constructor(datasetId, localStorageDir) {
        ...
        this.log = utils_log_1.default.child({ prefix: 'Dataset' });
        this.localStoragePath = path.resolve(path.join(localStorageDir, exports.LOCAL_STORAGE_SUBDIR, datasetId));
        this.counter = null;
        this.datasetId = datasetId;
        this.createdAt = null;
    ...
    _initialize() {
        return utils_2.ensureDirExists(this.localStoragePath)
    ...
    async getData(opts = {}) {
        ...
        utils_1.checkParamOrThrow(opts.limit, 'opts.limit', 'Maybe Number');
        utils_1.checkParamOrThrow(opts.offset, 'opts.offset', 'Maybe Number');
        utils_1.checkParamOrThrow(opts.desc, 'opts.desc', 'Maybe Boolean');
        ...
        if (!opts.limit)
            opts.limit = exports.LOCAL_GET_ITEMS_DEFAULT_LIMIT;
        if (!opts.offset)
            opts.offset = 0;
    ...
    async getInfo() {
        ...
        const name = id === consts_1.ENV_VARS.DEFAULT_DATASET_ID ? null : id;
        const result = {
            id,
            name,
    ...
    _getItemIndexes(offset = 0, limit = this.counter) {
        if (limit === null)
            throw new Error('DatasetLocal must be initialized before calling this._getItemIndexes()!');
...
exports.openDataset = (datasetIdOrName, options = {}) => {
    utils_1.checkParamOrThrow(datasetIdOrName, 'datasetIdOrName', 'Maybe String');
    utils_1.checkParamOrThrow(options, 'options', 'Object');
    utils_2.ensureTokenOrLocalStorageEnvExists('dataset');
    ...
    return process.env[consts_1.ENV_VARS.LOCAL_STORAGE_DIR] && !forceCloud
        ? utils_2.openLocalStorage(datasetIdOrName, consts_1.ENV_VARS.DEFAULT_DATASET_ID, DatasetLocal, datasetsCache)
        : utils_2.openRemoteStorage(datasetIdOrName, consts_1.ENV_VARS.DEFAULT_DATASET_ID, Dataset, datasetsCache, getOrCreateDataset);
};
/**
 ...
 * @function
 */
exports.pushData = item => exports.openDataset().then(dataset => dataset.pushData(item));
/**
