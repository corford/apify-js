...
const getPuppeteerOrThrow = (puppeteerModule = 'puppeteer') => {
    utils_1.checkParamOrThrow(puppeteerModule, 'puppeteerModule', 'String|Object');
...
exports.launchPuppeteer = async (options = {}) => {
    ...
    utils_1.checkParamOrThrow(options.args, 'options.args', 'Maybe [String]');
    utils_1.checkParamOrThrow(options.proxyUrl, 'options.proxyUrl', 'Maybe String');
    utils_1.checkParamOrThrow(options.puppeteerModule, 'options.puppeteerModule', 'Maybe String|Object');
    utils_1.checkParamOrThrow(options.stealth, 'options.stealth', 'Maybe Boolean');
    utils_1.checkParamOrThrow(options.stealthOptions, 'options.stealthOptions', 'Maybe Object');
    if (options.liveView || options.liveViewOptions) {
        utils_log_1.default.deprecated('Live view is no longer available in Apify.launchPuppeteer() and launchPuppeteerOptions. '
    ...
    if (optsCopy.useChrome && (optsCopy.executablePath === undefined || optsCopy.executablePath === null)) {
        optsCopy.executablePath = process.env[consts_1.ENV_VARS.CHROME_EXECUTABLE_PATH] || utils_2.getTypicalChromeExecutablePath();
    }
